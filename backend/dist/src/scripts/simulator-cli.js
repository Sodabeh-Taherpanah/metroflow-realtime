...
function parseArgs(argv) {
    ...
    return {
        ...
        interval: Number.isFinite(interval) && interval > 0 ? interval : defaults.interval,
        loop: loopRaw === undefined ? defaults.loop : loopRaw !== "false",
        agents: Number.isFinite(agents) && agents > 0
            ? Math.floor(agents)
            : defaults.agents,
        url: args.get("url") || defaults.url,
    };
}
function loadSamples(routeId) {
    const dataDir = process.env.DATA_DIR || "./data";
    const samplePath = (0, path_1.resolve)(process.cwd(), dataDir, "samples", `route-${routeId}-samples.json`);
    if (!(0, fs_1.existsSync)(samplePath)) {
        console.log(`[sim-cli] sample file not found at ${samplePath}, using one fallback point.`);
        return [
            {
                seq: 0,
                lat: 52.52,
    ...
    const file = JSON.parse((0, fs_1.readFileSync)(samplePath, "utf-8"));
...
function startSimulator(options) {
    ...
    socket.on("connect", () => {
        ...
        console.log(`[sim-cli] route=${options.route} interval=${options.interval}ms loop=${options.loop} agents=${options.agents} samples=${samples.length}`);
        const timer = setInterval(() => {
            for (let agentIndex = 0; agentIndex < options.agents; agentIndex += 1) {
                ...
                console.log(`[sim-cli] emit agent=${payload.agentId} seq=${payload.seq} lat=${payload.location.lat} lng=${payload.location.lng}`);
            }
            tick += 1;
...
//# sourceMappingURL=simulator-cli.js.map