...
export class IngestService {
  ...
  queueProbe(input: ProbeInput): { jobId: string; status: 'queued' } {
    ...
    this.appendJobLog(
      jobId,
      `queued gtfsZipPath=${input.gtfsZipPath || 'none'} gpxFile=${input.gpxFile?.originalname || 'none'}`,
    );
  ...
  getProbeLogs(
    ...
    const max = Number.isFinite(limit as number)
  ...
  resolveDownloadPath(input: {
    ...
    if (view === 'raw') {
      if (!route.rawPath || !existsSync(route.rawPath)) {
        throw new NotFoundException(
          `Raw artifact not found for route ${routeId}.`,
        );
      }
      return {
    ...
    if (view === 'canonical') {
      if (!route.canonicalPath || !existsSync(route.canonicalPath)) {
        throw new NotFoundException(
          `Canonical artifact not found for route ${routeId}.`,
        );
      }
      return {
    ...
    if (!existsSync(outputPath)) {
      throw new NotFoundException(
        `Sample output not found for route ${routeId}.`,
      );
    }
  ...
  private processProbeJob(jobId: string, input: ProbeInput) {
    ...
    this.appendJobLog(
      jobId,
      `started gtfsZipPath=${input.gtfsZipPath || 'none'} gpxFile=${input.gpxFile?.originalname || 'none'}`,
    );
    ...
    try {
      ...
      this.appendJobLog(
        jobId,
        `completed source=${result.sourceType} shapes=${result.stats.shapeCount} points=${result.stats.pointCount} canonicalPoints=${result.stats.canonicalPointCount}`,
      );
    } catch (error) {
      const message =
  ...
  private appendJobLog(jobId: string, message: string) {
  ...
  private probeGpx(
    gpxFile: GpxUploadFile | undefined,
